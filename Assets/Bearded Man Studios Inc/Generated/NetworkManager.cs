...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] SyncedBallNetworkObject = null;
		public GameObject[] SyncedCupNetworkObject = null;
		public GameObject[] SyncedPlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is SyncedBallNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SyncedBallNetworkObject.Length > 0 && SyncedBallNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SyncedBallNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<SyncedBallBehavior>();
						}
					}
			...
			else if (obj is SyncedCupNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SyncedCupNetworkObject.Length > 0 && SyncedCupNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SyncedCupNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<SyncedCupBehavior>();
						}
					}
			...
			else if (obj is SyncedPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SyncedPlayerNetworkObject.Length > 0 && SyncedPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SyncedPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<SyncedPlayerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateSyncedBall instead, its shorter and easier to type out ;)")]
		public SyncedBallBehavior InstantiateSyncedBallNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SyncedBallNetworkObject[index]);
			var netBehavior = go.GetComponent<SyncedBallBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SyncedBallBehavior>().networkObject = (SyncedBallNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateSyncedCup instead, its shorter and easier to type out ;)")]
		public SyncedCupBehavior InstantiateSyncedCupNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SyncedCupNetworkObject[index]);
			var netBehavior = go.GetComponent<SyncedCupBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SyncedCupBehavior>().networkObject = (SyncedCupNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateSyncedPlayer instead, its shorter and easier to type out ;)")]
		public SyncedPlayerBehavior InstantiateSyncedPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SyncedPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<SyncedPlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SyncedPlayerBehavior>().networkObject = (SyncedPlayerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of SyncedBall
		/// </summary>
		/// <returns>
		/// A local instance of SyncedBallBehavior
		/// </returns>
		/// <param name="index">The index of the SyncedBall prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public SyncedBallBehavior InstantiateSyncedBall(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SyncedBallNetworkObject[index]);
			var netBehavior = go.GetComponent<SyncedBallBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of SyncedCup
		/// </summary>
		/// <returns>
		/// A local instance of SyncedCupBehavior
		/// </returns>
		/// <param name="index">The index of the SyncedCup prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public SyncedCupBehavior InstantiateSyncedCup(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SyncedCupNetworkObject[index]);
			var netBehavior = go.GetComponent<SyncedCupBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of SyncedPlayer
		/// </summary>
		/// <returns>
		/// A local instance of SyncedPlayerBehavior
		/// </returns>
		/// <param name="index">The index of the SyncedPlayer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public SyncedPlayerBehavior InstantiateSyncedPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SyncedPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<SyncedPlayerBehavior>();
